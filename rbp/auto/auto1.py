...
def forward(c: controller.Controller, distance_cm: float, base_speed: int = 60):
    ...
    ticks_done_left, ticks_done_right = 0, 0
    ...
    while ticks_done_left < ticks_to_move or ticks_done_right < ticks_to_move:
        ...
        position[0] += ticks_to_distance((left_ticks + right_ticks) / 2) * cos(
            radians(direction)
        )
        position[1] += ticks_to_distance((left_ticks + right_ticks) / 2) * sin(
            radians(direction)
        )
