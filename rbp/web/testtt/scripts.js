...
function activateManualMode() {
	...
	xhr.open("POST", "http://robotpi-32.enst.fr:8080/api/mode?manual", true);
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState === 4 && xhr.status === 200) {
			...
			console.error("Erreur lors de l'activation du mode manuel");
		}
	};
	xhr.send();
}
...
function activateAutomaticMode() {
	const xhr = new XMLHttpRequest();
	xhr.open("POST", "http://robotpi-32.enst.fr:8080/api/mode?automatic", true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function sendCommand(direction) {
	const xhr = new XMLHttpRequest();
	xhr.open("POST", `http://137.194.173.32:8080/api/move?${direction}`, true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function activateMode(mode) {
	if (mode === 'navigate') {
		...
		document.getElementById('status').innerText = 'Mode: Navigate to Specific Case';
...
function sendNavigateRequest() {
	...
	const xhr = new XMLHttpRequest();
	xhr.open("POST", `http://137.194.173.32:8080/api/start?=navigate&start=${startCell}&target=${targetCell}`, true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	xhr.onreadystatechange = function () {
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
				document.getElementById('status').innerText = `Mode: Navigating from ${startCell} to ${targetCell}`;
...
function sendModeRequest(mode) {
	const xhr = new XMLHttpRequest();
	xhr.open("POST", `http://137.194.173.32:8080/api/automatic_mode?${mode}`, true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	xhr.onreadystatechange = function () {
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
				document.getElementById('status').innerText = `Mode: ${mode === 'flaghunt' ? 'Flag Hunt Active' : 'Active'}`;
